...
export default function SignUpForm() {
  ...
  const handleSignUpWithKakao = async () => {
    setIsLoading(true);
    setError(null);
    try {
      const { error: authError } = await authAPI.signInWithKakao();
      if (authError) {
        setError(authError.message);
      }
    } catch (err) {
      setError("Failed to sign up with Kakao");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSignUpWithEmail = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: authError } = await authAPI.signUpWithEmail(
        formData.email,
        formData.password,
        {
      ...
      if (authError) {
        ...
        // 회원가입 성공 - 확인 메일 발송 또는 로그인 페이지로 이동
        navigate("/signin");
      }
    } catch (err) {
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-5">
              ...
              >
                ...
                >
                  ...
                  <path
                    d="M10 3C6.13 3 3 5.13 3 7.77C3 9.42 4.07 10.88 5.62 11.62L5.2 13.77C5.1 14.37 5.75 14.87 6.3 14.52L8.77 12.77C9.18 12.8 9.59 12.82 10 12.82C13.87 12.82 17 10.69 17 8.05C17 5.41 13.87 3 10 3Z"
                    fill="#000"
                  />
                </svg>
                Sign up with Kakao
